...
from std_msgs.msg import Float64 #for moving the joints of gripper
...
def goal_detection(image):
    ...
    hsv_goal = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) #converting to hsv
    upper_blue = np.array([ 206.9, 82, 28]) #lower limit of blue bgr
    lower_blue = np.array([ 206.9, 82, 16]) #upper limit of blue bgr

    mask = cv2.inRange(hsv_goal, lower_blue, upper_blue) #masking the image frame
    goal_mask= cv2.moments(mask)
    if goal_mask['m00'] > 0: #finding the center of the location of the goal
        x,y,w,h= cv2.boundingRect(mask)
        x_loc= int(goal_mask['m10']/goal_mask['m00']) #x coordinate of the goal
        y_loc= int(goal_mask['m01']/goal_mask['m00']) #y coordinate of the goal
...
def image_callback(msg):
    ...
    #mask = cv2.inRange(hsv, lower_yellow, upper_yellow) #masking the image frame
...
rospy.init_node('Red_ball_Node') #starting the node
